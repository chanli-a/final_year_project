...
# # ─── 改进方案：多线程独立生成子csv，最后合并 ─────────────────────────────────────
# def run_and_save(rep_id):
#     rep_id, struct, raw_rows, traj_rows = run_single_full_raw(rep_id)

#     raw_file = os.path.join(outdir, f'raw_eq_100_part_{rep_id}.csv')
#     traj_file = os.path.join(outdir, f'traj_100_part_{rep_id}.csv')

#     pd.DataFrame(raw_rows).to_csv(raw_file, index=False)
#     pd.DataFrame(traj_rows).to_csv(traj_file, index=False)

#     return rep_id, struct

# def concat_and_cleanup(prefix, final_name):
#     files = sorted(f for f in os.listdir(outdir) if f.startswith(prefix))
#     full_paths = [os.path.join(outdir, f) for f in files]

#     # 合并 CSV 文件
#     combined_df = pd.concat([pd.read_csv(f) for f in full_paths])
#     combined_df.to_csv(os.path.join(outdir, final_name), index=False)
...
#     with mp.Pool(num_processes) as pool:
#         results = pool.map(run_and_save, full_ids)

#     # 保存结构参数
#     all_struct = {rep_id: struct for rep_id, struct in results}
#     with open(paramfile, 'wb') as f:
#         pickle.dump(all_struct, f)

#     # 合并并清理子文件
#     concat_and_cleanup("raw_eq_100_part_", "raw_eq_100.csv")
#     concat_and_cleanup("traj_100_part_", "traj_100.csv")
...
# # 主文件1运行完生成数据后才能运行主文件2
# # ─── Config paths ───────────────────────────────────
# # 定义文件路径常量（字符串类型）
# RAW_EQ_CSV   = 'output/raw_eq_100.csv'  # 原始平衡态数据文件路径
# TRAJ_CSV     = 'output/traj_100.csv'    # 轨迹数据文件路径
# PICKLE_FILE  = 'output/structural_params_all.pkl'  # 参数字典pickle文件路径
# OUT_MET_CSV  = 'output/metrics_100.csv'  # 输出指标文件路径

# # ─── Load data ──────────────────────────────────────
# # 使用pandas读取CSV文件（返回DataFrame对象）
# df_eq   = pd.read_csv(RAW_EQ_CSV)  # 读取平衡态数据
# df_traj = pd.read_csv(TRAJ_CSV)    # 读取轨迹数据

# # 加载pickle文件中的参数字典（字典结构，键为replicate_id，值为参数字典）
# with open(PICKLE_FILE, 'rb') as f:
#     structural_all = pickle.load(f)

# # 获取唯一的重复实验ID和温度值（numpy数组）
# reps  = sorted(df_eq['replicate'].unique())  # 排序后的重复实验ID列表
# temps = sorted(df_eq['T_K'].unique())       # 排序后的温度值列表(Kelvin)

# # ─── Precompute temp scalars ───────────────────────
# # 预计算温度缩放因子（字典推导式，键为温度，值为(N,2)的temp_p矩阵）
# temp_scalars = {
...
# #     'L': 泄漏率（长度N的向量）
# # 输出：
# #   temp_p: (N,2)矩阵，第1列为摄取率缩放因子，第2列为呼吸率缩放因子
...
# # 主循环：遍历每个重复实验和温度
# for rep in reps:  # rep: 当前重复实验ID（整数）
#     structural = structural_all[rep]  # 获取当前重复的结构参数（字典）

#     # 筛选当前重复的数据并设置温度索引
#     df_rep_eq   = df_eq[df_eq['replicate'] == rep].set_index('T_K')  # 平衡态DataFrame
#     df_rep_traj = df_traj[df_traj['replicate'] == rep].set_index('T_K')  # 轨迹DataFrame

#     for T_K in temps:  # T_K: 当前温度（浮点数，单位Kelvin）
#         # 获取当前温度下的平衡态数据行
#         row_eq = df_rep_eq.loc[T_K]  # pandas Series对象
        
#         # 提取消费者和资源平衡浓度（长度N和M的numpy数组）
#         Cmi    = row_eq.filter(like='Cmi_').values  # MiCRM消费者平衡浓度
#         Clv    = row_eq.filter(like='Clv_').values  # GLV消费者平衡浓度
#         Rmi    = row_eq.filter(like='Rmi_').values  # MiCRM资源平衡浓度
#         T_C    = T_K - 273.15  # 转换为摄氏度（浮点数）
#         t_eq_mi = df_rep_traj.loc[T_K, 't_eq_mi']  # MiCRM平衡时间（浮点数）

#         # 计算模型间差异指标
#         ErrEqAb, overlap = err_eq_and_overlap(Clv, Cmi)  # 平衡态丰度误差和物种重叠数
#         # err_eq_and_overlap输入：
#         #   C_LV_eq, C_MiCRM_eq: 长度N的丰度向量
#         # 输出：
#         #   equilibrium_error: 标量（平均对数比率误差）
#         #   overlap_count: 整数（共存物种数）

#         jacc = jaccard_index(Clv, Cmi, thresh=1e-6)  # Jaccard相似性指数（标量）
#         # jaccard_index输入：
#         #   G, M: 长度N的丰度向量
#         #   thresh: 存活阈值（标量）
#         # 输出：0-1之间的相似性指数（浮点数）

#         sh_mi, sh_lv = shannon(Cmi), shannon(Clv)  # 计算两个模型的香农多样性（标量）
#         # shannon输入：长度N的丰度向量
#         # 输出：非负多样性指数（浮点数）

#         bc = bray_curtis_dissimilarity(Clv, Cmi)  # Bray-Curtis相异性指数（标量）
#         # bray_curtis_dissimilarity输入：两个长度N的丰度向量
#         # 输出：0-1之间的相异性指数（浮点数）

#         # 构建温度修正后的参数字典
#         temp_p = temp_scalars[T_K]  # (N,2)矩阵
#         pT = {  # 参数字典
#             **structural,  # 解包原始参数
#             'u': structural['u'] * temp_p[:, 0][:, None],  # 温度修正的摄取矩阵(N,M)
#             'm': temp_p[:, 1],  # 温度修正的死亡率(N,)
#             'lambda': structural['l'].sum(axis=2),  # 总泄漏率(N,M)
#             'T': T_K  # 当前温度
#         }

#         # 创建虚拟解决方案对象存储平衡态数据
#         sol_eq = SolDummy()
#         sol_eq.y = np.concatenate([Cmi, Rmi])[:, None]  # (N+M,1)状态向量
#         sol_eq.t = np.array([t_eq_mi])  # 平衡时间数组

#         # 计算GLV有效参数和雅可比矩阵
#         p_lv = eff_LV_params(pT, sol_eq, verbose=False)  # 计算有效LV参数（字典）
#         # eff_LV_params输入：
#         #   p: MiCRM参数字典
#         #   sol: 包含.y属性的解对象
#         # 输出：包含'r'和'alpha'等键的字典

#         J_glv = eff_LV_jac(p_lv, sol_eq)  # GLV雅可比矩阵(N,N)
#         # eff_LV_jac输入：
#         #   p_lv: LV参数字典
#         #   sol: 解对象
#         #   threshold: 存活阈值
#         # 输出：(n,n)雅可比矩阵（n为存活物种数）

#         # 计算稳定性指标
#         stab_glv = leading_eigenvalue(J_glv)  # GLV主特征值（复数）
#         react_glv = leading_hermitian_eigenvalue(J_glv)  # GLV反应性（实数）

#         # 计算MiCRM雅可比矩阵和稳定性指标
#         J_mic = MiCRM_jac(pT, sol_eq)  # (N+M,N+M)雅可比矩阵
#         stab_mic = leading_eigenvalue(J_mic)  # MiCRM主特征值
#         react_mic = leading_hermitian_eigenvalue(J_mic)  # MiCRM反应性
...
#         #   tau_C: 消费者最快时间尺度（标量）
#         #   tau_R: 资源最慢时间尺度（标量）
#         #   eps: 分离系数tau_C/tau_R（标量）

#         log10_eps_teq = np.log10(eps / t_eq_mi)  # 无量纲分离系数（标量）
...
#         # 计算非正规性指标
#         comm = J_mic @ J_mic.T - J_mic.T @ J_mic  # 交换子矩阵(N+M,N+M)
#         nnorm = np.linalg.norm(comm, ord='fro')  # Frobenius范数（标量）
...
#             'T_K': T_K,  # 温度(K)
#             'T_C': T_C,  # 温度(°C)
#             'ErrEqAb': ErrEqAb,  # 平衡态丰度误差
#             'overlap': overlap,  # 共存物种数
#             'jaccard': jacc,  # Jaccard指数
#             'shannon_mi': sh_mi,  # MiCRM香农多样性
#             'shannon_lv': sh_lv,  # GLV香农多样性
#             'bray_curtis': bc,  # Bray-Curtis相异性
#             'stab_glv': stab_glv,  # GLV稳定性
#             'stab_mi': stab_mic,  # MiCRM稳定性
#             'abs_stab_err': abs(stab_glv - stab_mic),  # 稳定性绝对误差
#             'react_glv': react_glv,  # GLV反应性
#             'react_mi': react_mic,  # MiCRM反应性
#             'abs_react_err': abs(react_glv - react_mic),  # 反应性绝对误差
#             'tau_C': tau_C,  # 消费者时间尺度
#             'tau_R': tau_R,  # 资源时间尺度
#             'epsilon': eps,  # 时间尺度分离系数
#             't_eq_mi': t_eq_mi,  # MiCRM平衡时间
#             'log10_eps_t_eq': log10_eps_teq,  # 对数尺度分离系数
#             'hessian_norm': hnorm,  # Hessian范数
#             'non_normality': nnorm  # 非正规性指标
#         })
...
# def run_and_return(rep_id):
#     return run_single_full_raw(rep_id)  # 应返回 (rep_id, struct, raw_rows, traj_rows)

# def analyze_single_result(rep_id, struct, raw_rows, traj_rows):
#     df_eq = pd.DataFrame(raw_rows).set_index('T_K')
#     df_traj = pd.DataFrame(traj_rows).set_index('T_K')

#     temps = sorted(df_eq.index.unique())
#     temp_scalars = {
#         T_K: temp_trait(N, {'T': T_K, 'Tr': 273.15 + 10, 'Ed': 3.5, 'rho_t': np.array([0.0, 0.0]), 'L': np.full(N, 0.3)})[0]
#         for T_K in temps
#     }
...
#     for T_K in temps:
#         row_eq = df_eq.loc[T_K]
#         if isinstance(row_eq, pd.DataFrame):  # 多行情况
#             row_eq = row_eq.iloc[0]

#         Cmi = row_eq.filter(like='Cmi_').values
#         Clv = row_eq.filter(like='Clv_').values
#         Rmi = row_eq.filter(like='Rmi_').values
#         T_C = T_K - 273.15
#         t_eq_mi = df_traj.loc[T_K, 't_eq_mi']
...
#     with mp.Pool(num_processes) as pool:
#         all_results = pool.map(run_and_return, full_ids)
...
#     for rep_id, struct, raw_rows, traj_rows in all_results:
#         all_struct[rep_id] = struct
#         raw_full.extend(raw_rows)
#         traj_full.extend(traj_rows)

#         analyzed = analyze_single_result(rep_id, struct, raw_rows, traj_rows)
#         metrics.extend(analyzed)
